...
INSTRUCTIONS:
    ...
    value should be in the form of a nested dictionary
"""

# 1. CONSTANTS
FILENAME = 'store_file_math.txt'
MODE = 'r'
FORMAT_SPEC_5d = '5d'
...
def main():
    ...
    for line in entry_file:
        ...
        stock[prod_code] = {'quantity': stock_qty, 'price': unit_price}
    print('Values loaded into the dictionary (nested dictionary format)')
    print(stock)
    print('Display of data in table format')
    total_sum = 0
    for prod_code, data in stock.items():
        total_per_product = data['quantity'] * data['price']
        total_sum += total_per_product
        print(f' {prod_code}: {data['quantity']:{FORMAT_SPEC_5d}} x {data['price']:{FORMAT_SPEC_6_2f}} '
